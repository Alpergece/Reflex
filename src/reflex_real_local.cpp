...
float init_noise_var = 100;
float process_noise_var = 0;
float obs_noise_var = 10;

// KalmanFilter* kf;
KalmanFilter KalmanFilter(state_dim, obs_dim, control_dim, init_state, init_noise_var, process_noise_var, obs_noise_var);
// kf = KalmanFilter(state_dim, obs_dim, control_dim, init_state, init_noise_var, process_noise_var, obs_noise_var);
// <---

filters::IIR *force_filter;
filters::IIR *force_filter_l;
...
void getRightJTarCb(const trajectory_msgs::JointTrajectory::ConstPtr &msg){
    ...
    sendRightCmd(des_right_joint_pos,des_right_joint_pos);
...
void getLeftJTarCb(const trajectory_msgs::JointTrajectory::ConstPtr &msg){
    ...
    sendLeftCmd(des_left_joint_pos,des_left_joint_pos);
...
void sendRightCmd(const VectorXd pos, const VectorXd vel){
    ...
    // joint_right.points[0].positions[0]=0.0;
    // joint_right.points[0].positions[1]=0.7;
    // joint_right.points[0].positions[2]=1.53;
    // joint_right.points[0].positions[3]=1.56;
    // joint_right.points[0].positions[4]=-0.66;
    // joint_right.points[0].velocities[0]=0.0;
    // joint_right.points[0].velocities[1]=0.0;
    // joint_right.points[0].velocities[2]=0.0;
    // joint_right.points[0].velocities[3]=0.0;
    // joint_right.points[0].velocities[4]=0.0;
    right_command.publish(joint_right);
}

void sendLeftCmd(const VectorXd pos, const VectorXd vel){
    force_filter_l->update(pos);
    pos_filter_l = force_filter_l->get_iir();
    for (int j = 0; j < 7; ++j){
        joint_left.points[0].positions[j] = pos_filter_l[j];
...
void init_(){
    ...
    joint_right.points[0].time_from_start = ros::Duration(0.1);
    ...
    joint_left.points[0].time_from_start = ros::Duration(0.1);
    ...
    force_filter_l = new filters::IIR(7, 2, b_coef, a_coef);
    VectorXd pos_init(7);
    pos_init<<-0.66,-0.60,1.38,1.43,1.29,-0.71,-1.03;
    // pos_init<<-0.58, -0.09, 1.16, 1.49, 1.44, -0.68, -1.28;

    for(int i=0;i<1000;i++){
        force_filter->update(pos_init);
        pos_filter = force_filter->get_iir();
    }
    for(int j=0;j<1000;j++){
        force_filter_l->update(pos_init);
        pos_filter_l = force_filter_l->get_iir();
    }
